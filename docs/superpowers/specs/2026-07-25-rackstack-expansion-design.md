...
**Date:** 2026-07-25 **Status:** Approved by owner (NeverEndingCode) after section-by-section review. **Scope:** Four phased releases (v1.2 → v1.5) expanding RackStack into a long-term, multi-year idle game, inspired by Adventure Communist and ISEPS. FLOPS remains the primary currency throughout.
...
- New singleton `config` DB row holding one versioned JSON balance document; `config\_history` table records every edit for one-click rollback.
...
- **Canonical state** lives in the `saves` table (`data` = canonical `\{run, meta\}`); `last\_save` is reinterpreted as `lastEvaluatedAt`.
...
- **Minigames:** `POST /api/minigame/start` issues a session (uuid, game, started\_at); `POST /api/minigame/finish` accepts one result per session within game duration + a 10s grace window, clamped by config plausibility bounds (max taps/sec, max pairs, score ceilings) and enforces the per-game win cooldown server-side.
...
- `users.roles` — JSON array column, default `\[\]`. Values: `admin`, `event\_coordinator`.

- Owner grants/revokes `admin`; owner+admins grant/revoke `event\_coordinator`.

- Super admin(s) ids should be added to the Docker variable so it’s not hardcoded.  Allow it to be a comma separated list
...
### 3.7 Profile & identity

- **Custom usernames:** players set/change their username in Profile → Settings. Validation: 3–20 chars, `A–Z a–z 0–9 _ -`. **Uniqueness is DB-enforced** via a case-insensitive unique index on `users.username`; a taken name is rejected with a clear error. Endpoint: `PUT /api/me/username`.
- Today `upsertUser` overwrites the username from the OAuth provider on every login — a new `users.custom\_username` flag stops that sync once a player has chosen their own name.
- **Migration:** before the unique index is added, existing duplicate usernames are de-duplicated by suffixing (`name-2`), so the index creation can never fail on live data.
- **Version display:** the profile shows the running app version (baked in at build time from `package.json`); tapping it opens a version-history modal rendered from a `CHANGELOG.md` that ships in the image and gains an entry per release, starting with v1.2.
...
Permanent — survives Migrate **and** Singularity. ~8 upgrades, values in config: Compression Codecs (+% tape rewards) · Robot Arm (block time 6h → floor ~4h) · Priority Spin-up (offline job speed) · Head Start (reset grants first N blocks instantly) · Cold Fusion (global FLOPS mult) · Heat-Sink Tapes (+heat capacity) · Deep Uptime (+offline production cap hours).
...
`live\_events` table: identity (name, description, icon/color), `modifiers` (config overlay), `ladder` (10–20 rungs of `\{metric, target, reward\}`), window, status (`draft → scheduled → active → ended`), recurrence (optional annual month/day + duration).

- Ladder metrics are event-scoped deltas (FLOPS earned, minigames won, blocks claimed, tapes earned since joining). The server snapshots baseline counters at the user's personal event start (first login during the active window, §5.3); progress = current − baseline.
...
- Scheduler on boot + hourly: activates arrived windows, ends expired ones. **One globally active event enforced at activation**; personal windows may tail up to 24h past the global end (§5.3) but are force-ended by the next activation.

- Seeded with 4 recurring seasonal events: Summer Surge, Spooky Packets, Black Frame Friday, Frost Uptime.

- Permissions: owner/admin/`event\_coordinator` author drafts, schedule, trigger now, end early — from a new dashboard Events tab. Trigger-now with another event active requires explicitly ending it first.

- **Event leaderboard:** each event has its own leaderboard (rungs reached, event-metric progress) shown in the Event tab, backed by the `event\_participation` table (§5.3). A leaderboard **opt-out** toggle ships here in v1.4 and is reused by the v1.5 global boards.

### 5.3 Personal event windows & client UX

- **Personal start:** a user's event run starts at their **first login** (first authenticated `GET /api/state`) while the event is globally active — nobody loses ladder time to a start they didn't see. Ladder baselines snapshot at that personal start.
- **Personal duration:** the full event duration from the personal start, **capped at 24h past the event's global end** (join day 1 of a 7-day event → 7 days; join day 6 → until 24h after global end). If the next event activates, all lingering personal runs end immediately — one event per user at a time, always.
- **DB tracking for coordinators:** per-user participation lives in a new `event\_participation` table — `(user\_id, event\_id, started\_at, ends\_at, rungs\_claimed, last\_progress\_at)` — maintained by the reducer. The dashboard Events tab shows it per event (who's joined, how far they've climbed), and it doubles as the event leaderboard's data source.
- **Client UX:** event banner (surge-banner pattern) + an Event tab visible while the user's personal window is live: ladder, **personal** countdown, claim buttons, event leaderboard. Rung claims remain open for a **48h grace period** after the personal end.
...
`GET /api/leaderboard` (cached ~60s), aggregated server-side from canonical saves: all-time FLOPS (counter from §3.6), level, Legacy Cores, Singularities, Tapes, latest-event top rung. Respects the leaderboard **opt-out** toggle introduced with v1.4's event leaderboards. Rows show avatar, username, badges.
...
**New/changed tables:** `config` (singleton) · `config\_history` · `users.roles` + `users.custom\_username` (columns) + case-insensitive unique index on `users.username` · `minigame\_sessions` · `live\_events` (v1.4) · `event\_participation` (v1.4). `saves.data` becomes canonical state; `saves.last\_save` ⇒ `lastEvaluatedAt`. All changes additive; existing `CREATE TABLE IF NOT EXISTS` pattern plus guarded `ALTER TABLE`s (username de-dup runs before the unique index).
...
| `PUT /api/me/username` | user | Set/change username (unique, validated) |
...
| `GET/POST/PUT /api/admin/events`, `POST …/trigger`, `POST …/end`, `GET …/participation` | admin/coordinator | v1.4 event tooling incl. per-event participation view |
...
- Config `schemaVersion` auto-upgrade on boot; `config\_history` provides rollback.
