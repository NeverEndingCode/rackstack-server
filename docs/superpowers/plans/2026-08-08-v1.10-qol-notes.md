...
- Commits: d18f85d
- Changed: Added `meta.stats.bestLegacyCores` stat and `recordLegacyCorePeak()` helper; called from both `evaluate()` and `singularity()` as specified
- Decisions/deviations: none
- Tests: `TEST_BACKEND=sqlite npx vitest run tests/reducer.meta.test.js tests/state.test.js tests/gameRules.test.js` → 56 passed; specifically `npx vitest run tests/reducer.meta.test.js -t bestLegacyCores` → 5 passed
